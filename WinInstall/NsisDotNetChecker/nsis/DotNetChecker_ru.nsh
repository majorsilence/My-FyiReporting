...
${OrIf} $0 == "f"  ; if script is compiled in ANSI mode then we get only an "f"  https://github.com/ReVolly/NsisDotNetChecker/issues/4
		DetailPrint ".NET Framework $dotNetReadableVersion${FrameworkVersion} не найден, его необходимо установить перед запуском программы."
		Goto NoDotNET${FrameworkVersion}
	${Else}
		DetailPrint "Найден .NET Framework $dotNetReadableVersion${FrameworkVersion}, установка не требуется."
		Goto NewDotNET${FrameworkVersion}
	${EndIf}

NoDotNET${FrameworkVersion}:
	MessageBox MB_YESNOCANCEL|MB_ICONEXCLAMATION \
	".NET Framework не установлен. Требуемая версия: $dotNetReadableVersion${FrameworkVersion}.$\nСкачать .NET Framework $dotNetReadableVersion${FrameworkVersion} с сайта www.microsoft.com?" \
...
DownloadDotNET${FrameworkVersion}:
	DetailPrint "Скачиваем .NET Framework $dotNetReadableVersion${FrameworkVersion}."
	NSISDL::download $dotNetUrl${FrameworkVersion} "$TEMP\dotnetfx.exe"
	DetailPrint "Загрузка завершена."

	Pop $0
	${If} $0 == "cancel"
		MessageBox MB_YESNO|MB_ICONEXCLAMATION \
		"Загрузка отменена. Продолжить установку?" \
	...
	DetailPrint "Ожидаем выполнения скаченного установщика .NET Framework."
	ExecWait '$TEMP\dotnetfx.exe /q /c:"install /q"'

	DetailPrint "Установка\обновление .NET Framework завершена. Удаляем установщик .NET Framework."
	Delete "$TEMP\dotnetfx.exe"
	DetailPrint "Установщик .NET Framework удален."
	goto NewDotNet${FrameworkVersion}

GiveUpDotNET${FrameworkVersion}:
	Abort "Установка прервана пользователем."

NewDotNET${FrameworkVersion}:
	DetailPrint "Выполнение оставшейся части установки."
	Pop $7
	Pop $6
	Pop $5
	Pop $4
